...
static bool wifiCredentialsMountSd() {
    SPI.begin(40, 39, 14, 12);
    return SD.begin(12, SPI, 20000000) && SD.cardType() != CARD_NONE;
}

static String wifiCredentialsCleanValue(String value) {
    value.trim();
    if (value.length() >= 2) {
        char first = value[0];
...
static void wifiCredentialsParseLine(String line, String &ssid, String &pass) {
    line.trim();
    if (line == "" || line.startsWith("#")) return;

    int eq = line.indexOf('=');
    if (eq < 0) {
        if (ssid == "") ssid = wifiCredentialsCleanValue(line);
        else if (pass == "") pass = wifiCredentialsCleanValue(line);
        return;
    }
    ...
    key.trim();
    key.toLowerCase();
    String value = wifiCredentialsCleanValue(line.substring(eq + 1));

    if (key == "ssid" || key == "wifi_ssid" || key == "network") {
        ssid = value;
    } else if (key == "password" || key == "pass" || key == "wifi_pass" || key == "psk") {
        pass = value;
    }
}

static bool wifiCredentialsReadFile(const char* path, String &ssid, String &pass) {
    ...
    while (file.available()) {
        wifiCredentialsParseLine(file.readStringUntil('\n'), ssid, pass);
    }
    file.close();
    return ssid != "";
}
...
bool saveWifiCredentialsToSd() {
    ...
    if (!wifiCredentialsMountSd()) return false;
    if (!SD.exists(WIFI_CREDENTIALS_OS_DIR) && !SD.mkdir(WIFI_CREDENTIALS_OS_DIR)) return false;

    if (SD.exists(WIFI_CREDENTIALS_FILE)) SD.remove(WIFI_CREDENTIALS_FILE);
    File file = SD.open(WIFI_CREDENTIALS_FILE, FILE_WRITE);
    if (!file) return false;

    file.println("# Breach_OS Wi-Fi profile");
    file.println("# Generated by Breach_OS. Keep this file private.");
    file.print("ssid=");
    file.println(savedSSID);
    file.print("password=");
    file.println(savedWifiPass);
    file.close();
    return true;
}
...
bool loadWifiCredentialsFromSd() {
    ...
    String sdSsid;
    String sdPass;
    bool loaded = wifiCredentialsReadFile(WIFI_CREDENTIALS_FILE, sdSsid, sdPass);
    bool loadedLegacy = false;
    if (!loaded) {
        loadedLegacy = wifiCredentialsReadFile(WIFI_CREDENTIALS_LEGACY_FILE, sdSsid, sdPass);
        loaded = loadedLegacy;
    }
    if (!loaded) return false;

    savedSSID = sdSsid;
    savedWifiPass = sdPass;
    prefs.putString("wifi_ssid", savedSSID);
    prefs.putString("wifi_pass", savedWifiPass);

    if (loadedLegacy && saveWifiCredentialsToSd() && SD.exists(WIFI_CREDENTIALS_LEGACY_FILE)) {
        SD.remove(WIFI_CREDENTIALS_LEGACY_FILE);
    }
    return true;
}
...
void drawSplash() {
    ...
    if (showSplashBootMenu) {
        ...
        drawGlitchText("BOOT NODE", 72, 4, 1, CP_CYAN, true, true);
        drawTopStatusIcons(132, 1);
    ...
    if (savedSSID != "") {
        ...
        if (WiFi.status() == WL_CONNECTED) {
            ...
            if (resumeOtaAfterWifi) {
                ...
                resumeTextfilesAfterWifi = false;
                enterTextfilesMode();
            } else if (resumeApModeLanWebAfterWifi) {
                resumeApModeLanWebAfterWifi = false;
                enterApModeLanWeb();
